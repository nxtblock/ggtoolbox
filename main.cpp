...
float tfa = 0.0f;
string nextPage = "";
bool transitionFadeIn = true; // true: 淡出旧页面, false: 淡入新页面

void StartTransition(string page)
{
    if (transitionActive)
        return;
...
void UpdateTransition()
{
    ...
    DrawTextureEx(intro, {0, 0}, 0, 1.0f, Fade(WHITE, tfa));
}

void Sidebar()
{
    DrawMcRectangle(0, 0, SidebarHeight - 3, 720, GDARKGRAY);
    DrawTextureEx(logo, {SidebarHeight / 2 - 90, 20}, 0, 0.6f, WHITE);
...
void Sign()
{
    ...
    if (query_file("sign").find(to_string(month) + "-" + to_string(day)) == -1)
    {
        ...
        if (sday.size() == 1)
            sday = "0" + sday;
        DrawTextUTF(sday, {SidebarHeight + 90, 100}, 100, 2, WHITE);
        if (DrawMcButton({SidebarHeight + 35, 220}, 205, 50, "签到", DARKGREEN, 30) == 1)
        {
            random_device fate;
            update_file("sign", to_string(month) + "-" + to_string(day) + "-" + to_string(fate() % 9));
        }
    }
    else
    {
        ...
        if (last_fate == 0)
        {
            ...
            DrawTextUTF("[宜]万事皆宜", {SidebarHeight + 32, 200}, 15, 2, RED);
        }
        else if (last_fate == 1)
        {
            DrawTextUTF("$中吉$", {SidebarHeight + 32, 110}, 80, 2, RED);

            DrawTextUTF("[宜]打比赛", {SidebarHeight + 32, 200}, 20, 2, RED);
            DrawTextUTF("AK 全场", {SidebarHeight + 32, 220}, 15, 2, LIGHTGRAY);
            DrawTextUTF("[宜]玩 MC", {SidebarHeight + 32, 240}, 20, 2, RED);
            DrawTextUTF("挖到远古残骸", {SidebarHeight + 32, 260}, 15, 2, LIGHTGRAY);

            DrawTextUTF("[忌]坐地铁", {SidebarHeight + 132, 200}, 20, 2, WHITE);
            DrawTextUTF("地铁卡失灵", {SidebarHeight + 132, 220}, 15, 2, LIGHTGRAY);
            DrawTextUTF("[忌]去学术版", {SidebarHeight + 132, 240}, 20, 2, WHITE);
            DrawTextUTF("被认为是 XXS", {SidebarHeight + 132, 260}, 15, 2, LIGHTGRAY);
        }
        else if (last_fate == 2)
        {
            DrawTextUTF("$小吉$", {SidebarHeight + 32, 110}, 80, 2, RED);
            
            DrawTextUTF("[宜]ACwing", {SidebarHeight + 32, 200}, 20, 2, RED);
            DrawTextUTF("胜率100%", {SidebarHeight + 32, 220}, 15, 2, LIGHTGRAY);
            DrawTextUTF("[宜]去食堂", {SidebarHeight + 32, 240}, 20, 2, RED);
            DrawTextUTF("双倍菜量", {SidebarHeight + 32, 260}, 15, 2, LIGHTGRAY);

            DrawTextUTF("[忌]抄作业", {SidebarHeight + 132, 200}, 20, 2, WHITE);
            DrawTextUTF("被巡堂抓获", {SidebarHeight + 132, 220}, 15, 2, LIGHTGRAY);
            DrawTextUTF("[忌]喝冰红茶", {SidebarHeight + 132, 240}, 20, 2, WHITE);
            DrawTextUTF("谁在喝农药", {SidebarHeight + 132, 260}, 15, 2, LIGHTGRAY);
        }
        else if (last_fate == 3)
        {
            ...
            DrawTextUTF("[宜]玩Florr", {SidebarHeight + 32, 240}, 20, 2, RED);
            DrawTextUTF("抢到 Super", {SidebarHeight + 32, 260}, 15, 2, LIGHTGRAY);

            DrawTextUTF("[忌]重构代码", {SidebarHeight + 132, 200}, 20, 2, WHITE);
            DrawTextUTF("越改越崩", {SidebarHeight + 132, 220}, 15, 2, LIGHTGRAY);
            DrawTextUTF("[忌]看QQ", {SidebarHeight + 132, 240}, 20, 2, WHITE);
            DrawTextUTF("全是神人", {SidebarHeight + 132, 260}, 15, 2, LIGHTGRAY);
        }
        else if (last_fate == 4)
        {
            ...
            DrawTextUTF("WA on test 1", {SidebarHeight + 132, 220}, 15, 2, LIGHTGRAY);
            DrawTextUTF("[忌]点外卖", {SidebarHeight + 132, 240}, 20, 2, WHITE);
            DrawTextUTF("骑手正在\n吃你的外卖", {SidebarHeight + 132, 260}, 15, 2, LIGHTGRAY);
        }
        else if (last_fate == 5)
        {
            DrawTextUTF("$中平$", {SidebarHeight + 32, 110}, 80, 2, GREEN);
            ...
            DrawTextUTF("教练跟你一起看", {SidebarHeight + 132, 220}, 15, 2, LIGHTGRAY);
            DrawTextUTF("[忌]开新坑", {SidebarHeight + 132, 240}, 20, 2, WHITE);
            DrawTextUTF("旧坑还没填", {SidebarHeight + 132, 260}, 15, 2, LIGHTGRAY);
        }
        else if (last_fate == 6)
        {
            DrawTextUTF("$中平$", {SidebarHeight + 32, 110}, 80, 2, GREEN);
            ...
            DrawTextUTF("越调越差", {SidebarHeight + 132, 220}, 15, 2, LIGHTGRAY);
            DrawTextUTF("[忌]查成绩", {SidebarHeight + 132, 240}, 20, 2, WHITE);
            DrawTextUTF("挂科预警", {SidebarHeight + 132, 260}, 15, 2, LIGHTGRAY);
        }
        else if (last_fate == 7)
        {
...
void Home()
{
    ...
    DrawTextureEx(homepng, {SidebarHeight + 263, 87}, 0, 0.51f, Fade(WHITE, 0.7f));
...
{
    if (!filesystem::exists("../gsml-main/"))
        return;
    allFolders.clear();
...
void gsml()
{
    ...
    if (filteredFolders.size() == 0)
    {
        if(gsmliserror){
            DrawTextUTF("连接失败...", {SidebarHeight + 400, 300}, 40, 2, YELLOW);
            DrawTextUTF("你无法访问脚本市场并且无法自动更新",{SidebarHeight + 350, 350}, 20, 2, RED);
        }
        else{
            DrawTextUTF("连接中...", {SidebarHeight + 400, 300}, 40, 2, YELLOW);
        }
    }
    if(gsmliserror){
        showmsg("GSML 连接失败","可能是网络问题。并且你无法访问\n市场并且无法自动更新");
    }
